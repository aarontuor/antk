...
double0t = tf.constant(U.astype(numpy.float64))
...
print("binary tensor combine (outputsize 3): \nmat1 (shape = %s):\n%s\nmat2 (shape = %s)\n%s\nnewmat (shape=%s)\n%s"
      % (mat1.get_shape().as_list(),
         sess.run(mat1),
         mat2.get_shape().as_list(),
         sess.run(mat2),
         newmat.get_shape().as_list(),
         sess.run(newmat)))
...
t0minus = sess.run(mult0double)
print("mode 0 matrix multiply shape: %s    tensor: \n%s\n" % (t0minus.shape, t0minus))

t1minus = sess.run(mult1double)
print("mode 1 matrix multiply shape: %s    tensor: \n%s\n" % (t1minus.shape, t1minus))

tminus = sess.run(mult2double)
print("mode 2 matrix multiply shape: %s    tensor: \n%s\n" % (tminus.shape, tminus))
